#Python PEP style guide: https://www.python.org/dev/peps/pep-0008/#indentation
...
def type_explorer(typ, name="", depth=0, indent=4, parent=None):
    base = BaseTypeExplorer(typ)
    if base.type_name() == "dict":
        return DictionaryExplorer(
            typ, name, depth, indent, parent)
    elif base.type_name() == "list":
        return ListExplorer(
            typ, name, depth, indent, parent)
    else:
        return BaseTypeExplorer(
            typ, name, depth, indent, parent)


class BaseTypeExplorer():

    def __init__(self, typ, name="", depth=0,
                 indent=4, parent=None):
        self.typ = typ
        self.name = name
        self.depth = depth
        self.indent = indent
    ...
    def __str__(self):
        suffix = ""
        if len(self.name) > 0:
            suffix = " called {0}".format(self.name)
            type_str = "A {0}{1}".format(self.pretty_name(), suffix)
        return "{0}{1}".format(self.indent_str(), type_str)
...
class InterestingTypeExplorer(BaseTypeExplorer):
    ...
    def __init__(self, typ, name="", depth=0, indent=4,
                parent=None):
    ...
    def unique_boring_type_pretty_names_to_sentence(self):
        ...
        if len(pretty_names) == 0:
            ...
            return ", ".join(
                pretty_names[0:len(pretty_names) - 1])
            + " and " + pretty_names[len(pretty_names) - 1]
...
class DictionaryExplorer(InterestingTypeExplorer):
    ...
    def __str__(self):
        type_str = "{0}".format(self.name).strip()
        result = []
        result.append("{0}{1} {{".format(self.indent_str(), type_str))
        ...
        if len(self.interesting) > 0:
            prefix = "And some"
            result.append("{0}{1} keys holding {2}".format(self.indent_str(self.depth + 1), prefix, self.unique_boring_type_pretty_names_to_sentence()))
            result.append("{1}}}".format(self.indent_str()))
        return "\n".join(result)
